...
import com.fzcoder.opensource.blog.aop.annotation.*;
import com.fzcoder.opensource.blog.bean.ArticleDownloadConfigInfo;
...
import com.fzcoder.opensource.blog.utils.DateUtil;
import com.fzcoder.opensource.blog.utils.IdGenerator;
import com.fzcoder.opensource.blog.vo.ArticleView;
import com.fzcoder.opensource.blog.vo.Post;
import com.fzcoder.opensource.blog.vo.TagView;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.fzcoder.opensource.blog.entity.Article;
import com.fzcoder.opensource.blog.mapper.IArticleMapper;
...
@Slf4j
@Service
public class ArticleServiceImpl extends ServiceImpl<IArticleMapper, Article> implements ArticleService {
    @Autowired
    ...
    public boolean save(@RecordParam(RecordParamType.ENTITY) ArticleForm form,
        ...
        try {
            ...
            String formatDate = DateUtil.format(
                    date, DateUtil.FORMAT_PATTERN_2, DateUtil.TIMEZONE_UTC8E
            );
            // 设置文章创建时间
            article.setDate(formatDate);
            // 设置文章最后一次修改时间
            article.setUpdateTime(formatDate);
            // 将文章添加至数据库中
            boolean result = getBaseMapper().insert(article) > 0;
            // 处理文章标签
    ...
    public Post getViewById(Long id) {
        return getBaseMapper().selectPostById(id);
    }

    @Override
    public ArticleForm getFormById(Long id) {
        return new ArticleForm(getBaseMapper().selectById(id), tagService.listRelation(id));
    }
    ...
    public IPage<ArticleView> getPages(Integer uid, String keyword,
        ...
        return getBaseMapper().selectPages(new Page<>(pageNum, pageSize), queryWrapper);
    }
    ...
    public boolean update(@RecordParam(RecordParamType.ENTITY) ArticleForm form,
        ...
        // 更新文章并返回结果
        return getBaseMapper().updateById(article) > 0;
    }
    ...
    public boolean removeById(@RecordParam(RecordParamType.ENTITY) ArticleForm form,
        ...
        // 2.删除文章并返回结果
        return getBaseMapper().deleteById(form.getId()) > 0;
    }
}
